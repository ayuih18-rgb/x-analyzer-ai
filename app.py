...
def fetch_rss(url, timeout=10):
    """تجربة جميع الخوادم وإرجاع XML أو None"""
...
def check_shadowban(username):
    ...
    if search_xml is None:
        return "غير معروف", "تعذر فحص البحث (Nitter لا يستجيب)"
...
def analyze_full(username, tweets, do_compare=False):
    ...
    if "سلبي" in sentiment:
        score -= 10
        issues.append("مشاعر سلبية")
        fixes.append("شارك محتوى إيجابي")

    # فحص الحظر (يخصم النقاط فقط عند التأكد من الحظر)
    ...
    if do_compare and top_words:
        try:
            ...
            if similar:
                comp_data = compare_accounts(None, similar)  # لا نحتاج main_stats هنا
                if comp_data:
                    comparison = comp_data
                    for comp in comp_data:
                        if comp['avg_len'] > avg_len + 10:
                            fixes.append(f"متوسط طول تغريداتك أقل من @{comp['username']}، حاول زيادة التفاصيل")
                        if comp['hashtags'] > hashtags:
                            fixes.append(f"استخدم هاشتاغات أكثر مثل @{comp['username']}")
        except Exception:
            pass  # فشل المقارنة لا يوقف التحليل

    # حالة الحساب
    if score >= 80:
        status, risk = "🟢 قوي", "منخفض"
    elif score >= 60:
        status, risk = "🟡 متوسط", "متوسط"
    else:
        status, risk = "🔴 ضعيف", "مرتفع"
...
<form method="post">
  ...
  <div id="deep-panel" class="panel">
    <input name="deep_url" placeholder="رابط الحساب للمقارنة المتقدمة" value="{{ form_data.get('deep_url', '') }}">
    ...
    if request.method == "POST":
        ...
        if mode == "manual":
            ...
            if not url:
                ...
                tweets = parse_tweets_from_rss(xml, 10)
                if not tweets:
                    data = {"error": "تعذر جلب التغريدات من الحساب"}
                else:
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=10000)    try:
        root = ET.fromstring(xml_content)
        tweets = []
        for item in root.findall(".//item")[:max_items]:
            title = item.find("title")
        ...
        print(f"[XML] خطأ في تحليل تغريدات: {e}")
        return []
...
def analyze_full(username, tweets, do_compare=False):
    ...
    if do_compare and top_words:
        try:
            ...
            print(f"[مقارنة] فشل غير متوقع: {e}")
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=10000)                return resp.content
        except:
            continue
    return None
