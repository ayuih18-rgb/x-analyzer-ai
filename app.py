...
def search_tweets_fallback(username):
    ...
    # 2. بعض خوادم Nitter تدعم JSON للبحث
    for instance in NITTER_INSTANCES:
        try:
            resp = requests.get(
    ...
    # 3. خطة أخيرة: تحليل HTML صفحة البحث (بسيط)
    for instance in NITTER_INSTANCES:
        try:
            resp = requests.get(
            ...
            if resp.status_code == 200:
                # نبحث عن عناصر تحتوي على نص التغريدة
                # هذا Regex بسيط وقد لا يكون دقيقًا 100% لكنه كافٍ
...
def check_shadowban(username):
    ...
    if search_tweets is None:
        # إذا فشل كل شيء، نفترض أننا لا نستطيع التأكد
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=10000)        tweets = []
        for item in root.findall(".//item")[:max_items]:
            title = item.find("title")
            if title is not None and title.text:
                text = title.text.strip()
